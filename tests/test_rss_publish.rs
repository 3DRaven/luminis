...
async fn publish_mastodon_and_telegram_and_file_from_rss_with_cache() {
    ...
    // Setup mocks for this scenario: NPA error -> RSS fallback + Stages + DOCX + Gemini + Telegram + Mastodon
...
";
    ...
    assert!(decoded_body.contains("Губернаторы+смогут+передавать"), "Mastodon post should contain Gemini text about medical devices");
...
async fn publish_console_and_file_from_rss_with_cache() {
    ...
    // Setup mocks for this scenario: RSS + Stages + Gemini + DOCX
    mount_rss(&server, &rss_xml).await;
    mount_stages(&server, &stages_json).await;
    mount_docx(&server).await;
    mount_gemini_generate(&server).await;
